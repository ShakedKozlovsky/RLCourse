...
def report_to_json(report: GameReport) -> str:
    """Spec § 3.5 JSON shape, sorted, deterministic — string suitable as email body."""
    payload = asdict(report)
    # Convert datetime objects in sub_games to iso strings
...
def build_idempotency_key(report: GameReport) -> str:
    """SHA-256 of the deterministic JSON — same content → same id (ADR-010)."""
