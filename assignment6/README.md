...
18. **Live Tkinter GUI window (v1.12 — fixes prior §5.4 gap)** — `marl gui --checkpoint saved_models/qmix_curriculum.pt` opens a real Tkinter window where you watch the cop chase the thief step-by-step (Start / Reset / Quit buttons; configurable frame delay). Headless-safe: on machines without DISPLAY, exits with a clear message + a pointer to `assets/figures/sub_game.gif`. Widget code lives at [`src/marl_lab/interface/tk_gui.py`](src/marl_lab/interface/tk_gui.py); 3 tests verify the module imports cleanly and the launcher gives helpful errors when preconditions are missing.
19. **MCP HTTP transport for cross-network bonus matches (v1.12 — fixes prior v1.11 stub)** — `marl play-bonus --peer-mcp-url ...` now actually WORKS end-to-end via `src/marl_lab/mcp/http_transport.py::build_http_transport(url, token)`. Real `httpx.post` with Bearer auth + FastMCP payload/result wrapping; 5 tests verify the transport posts correctly, handles non-2xx, connection errors, and both wrapped/bare response shapes. When a partner group appears, one command starts the live inter-group match.
20. **Reflection Q3 empirical study (v1.12 — was previously unanswered)** — spec § 7.2 asks "how does the swarm-vs-single-agent framing change the optimal-policy story?" We built `src/marl_lab/environment/multi_cop_env.py` (N-cop pursuit env; N ≥ 1) and `scripts/q3_swarm_vs_single.py` (500-game random-policy study across N ∈ {1, 2, 3, 4}). Result: capture rate scales cleanly 47% → 68% → 80% → 90% as N grows — coordination-through-density is real and empirically demonstrable even with random policies. Figure at [`assets/figures/q3_swarm_vs_single.png`](assets/figures/q3_swarm_vs_single.png); raw data at `assets/logs/q3_swarm_vs_single.json`. 7 tests cover the multi-cop env.
21. **Training-and-eval CLI improvements (v1.12)** — `marl train --seed 42 --curriculum --checkpoint ...` with reproducible seeding + optional Lin-2025 grid-ramp; `scripts/evaluate_checkpoint.py --checkpoint ... --n 100` for pure greedy evaluation (used to catch the honest 0%-cop-win-rate finding, now documented in `FAILURE_MODES.md § 8`). Two new CLI tests including a determinism test (same seed → bit-identical Q-net weights).
