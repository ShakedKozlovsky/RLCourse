...
class RewardFunction:
    ...
    def _imbalance(state: np.ndarray) -> float:
        ...
        if total <= 0:
            return 0.0
        p = dist / total
        # entropy of p; 0 when concentrated, log(K) when uniform
        eps = 1e-12
        entropy = float(-(p * np.log(p + eps)).sum())
        # imbalance = 1 - normalised entropy
