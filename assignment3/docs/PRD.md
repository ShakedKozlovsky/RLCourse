...
| v8 (Layer 15) | Reward | REST now earns **zero gain** (not full volume reward). Layer 13's entropy sweep proved the prior reward made REST optimal — greedy A2C picked REST for 28 days straight. New formulation: `gain = volume if action != REST else 0`; REST still pays the overload penalty (rolling 7-day mean) and bypasses imbalance, so the policy learns to use REST strategically when overload is high. |
| v9 (Layer 15) | Algorithms | **PPO added** as a third algorithm completing the chain REINFORCE → A2C → PPO. PPO uses the same `ActorCriticNet` as A2C, the same env, and the same hyperparameter format — so the comparison isolates the learning rule (clipped surrogate + multiple epochs per batch). |
